...
// @Accept	application/json
// @Param	token	body string true "token"
// @Param	id 		body int 	true "operation id"
// @Produce application/json
...
// @Router	/api/v1/healthcheck/result [get]
func GetResultByOperationID(c *gin.Context) {
	...
	operationID, err := jsonparser.GetInt(data, operationIDJSON)
	if err != nil {
		resp.ResponseNOK(c, message.ErrFieldNotExistsOrWrongType, errors.Trace(err), operationIDJSON)
		return
	}
	// init service
	s := healthcheck.NewServiceWithDefault()
	// get entities
	err = s.GetResultByOperationID(int(operationID))
	if err != nil {
