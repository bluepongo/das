...
func testInitTableRepo() *TableRepo {
	conn, err := mysql.NewConn(testDASMySQLAddr, constant.EmptyString, viper.GetString(config.DBApplicationMySQLUserKey), viper.GetString(config.DBApplicationMySQLPassKey))
	if err != nil {
